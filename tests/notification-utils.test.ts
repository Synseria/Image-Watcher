...
describe('shouldSendNotification()', () => {
  const availableVersions = [parseSemver('1.1.0')!, parseSemver('1.2.0')!, parseSemver('2.0.0')!];

  describe("Première notification (pas d'historique)", () => {
    it('retourne toutes les versions si aucune notification précédente', () => {
      const result = shouldSendNotification(undefined, undefined, availableVersions);
    ...
    it('retourne toutes les versions si lastNotified est undefined', () => {
      const result = shouldSendNotification(undefined, '1.0.0', availableVersions);
    ...
    it('retourne toutes les versions si lastNotifiedVersion est undefined', () => {
      const result = shouldSendNotification(new Date(), undefined, availableVersions);
  ...
  describe('Filtrage des nouvelles versions', () => {
    it('filtre les versions plus récentes que la dernière notifiée', () => {
      ...
      const result = shouldSendNotification(lastNotified, '1.1.0', availableVersions);
    ...
    it('retourne un tableau vide si aucune version plus récente', () => {
      const lastNotified = new Date();
      const result = shouldSendNotification(lastNotified, '2.0.0', availableVersions);
    ...
    it('ignore la dernière version notifiée', () => {
      const lastNotified = new Date();
      const result = shouldSendNotification(lastNotified, '1.2.0', availableVersions);
  ...
  describe('Calcul du délai de rappel', () => {
    it('shouldRemind=true si 7+ jours depuis la dernière notification', () => {
      ...
      const result = shouldSendNotification(sevenDaysAgo, '1.0.0', availableVersions);
    ...
    it('shouldRemind=false si moins de 7 jours', () => {
      ...
      const result = shouldSendNotification(sixDaysAgo, '1.0.0', availableVersions);
    ...
    it("shouldRemind=false si notification aujourd'hui", () => {
      ...
      const result = shouldSendNotification(today, '1.0.0', availableVersions);
  ...
  describe('Gestion des versions invalides', () => {
    it("retourne toutes les versions si lastNotifiedVersion n'est pas semver", () => {
      const result = shouldSendNotification(new Date(), 'latest', availableVersions);

      expect(result.newVersions).toEqual(availableVersions);
    });

    it('gère une liste de versions vide', () => {
      const result = shouldSendNotification(new Date(), '1.0.0', []);
  ...
  describe('Scénarios réels', () => {
    it('notifie uniquement les nouvelles versions après rappel', () => {
      ...
      const result = shouldSendNotification(eightDaysAgo, '1.0.0', availableVersions);
    ...
    it('combine nouvelles versions + rappel', () => {
      ...
      const moreVersions = [...availableVersions, parseSemver('2.1.0')!, parseSemver('2.2.0')!];

      const result = shouldSendNotification(tenDaysAgo, '1.1.0', moreVersions);
    ...
    it('gère le passage à une version majeure', () => {
      ...
      const versions = [parseSemver('1.9.9')!, parseSemver('2.0.0')!];

      const result = shouldSendNotification(yesterday, '1.9.0', versions);
  ...
  describe('Cas limites temporels', () => {
    it('gère exactement 7 jours (limite incluse)', () => {
      ...
      const result = shouldSendNotification(exactlySeven, '1.0.0', availableVersions);
    ...
    it('gère les dates futures (ne devrait pas arriver)', () => {
      ...
      const result = shouldSendNotification(future, '1.0.0', availableVersions);
    ...
    it('gère les très anciennes notifications (30+ jours)', () => {
      ...
      const result = shouldSendNotification(longAgo, '1.0.0', availableVersions);
