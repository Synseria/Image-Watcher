  ...
  describe('getRelease()', () => {
    if (!IS_INTEGRATION) {
      it('récupère une release via GitHub', async () => {
        ...
        expect(mockGitHub!.getRelease).toHaveBeenCalledWith('owner/repo', 'v1.0.0', expect.any(Object));
      ...
      it('passe les options au provider', async () => {
        ...
        expect(mockGitHub!.getRelease).toHaveBeenCalledWith('owner/repo', 'v1.0.0', expect.objectContaining(options));
      });
